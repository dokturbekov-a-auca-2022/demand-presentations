...
(() => {
  const config = {"dir":"lesson-19-listening-airport-travelling","mode":"terminal","label":"GROUP TASK","colors":["#17c3b2","#ff754f","#08243d","#fff6df"],"labScene":20,"lab":{"title":"Airport announcement group task","prompt":"Listen to one announcement. Record the destination, gate, time or status, and required passenger action. Report all four details to the class.","roles":["Audio operator: play the clip without showing the transcript.","Number recorder: write the flight, gate, and time.","Action recorder: write what passengers must do.","Traveller: ask one clarification question."],"rounds":["1. Identify the main message","2. Record the key details","3. Report the passenger action"],"success":"The group reports the correct destination, gate, time or status, and passenger action."},"taskScenes":[2,4,7,8,10,12,13,14,16,17,18,20,22,24,25,28],"audio":[{"scene":8,"file":"gate-change.wav","label":"ANNOUNCEMENT A - MAIN IDEA AND DETAILS","text":"Attention, please. Flight two zero eight to Istanbul will now leave from Gate eighteen, not Gate fourteen. Boarding begins at two thirty-five. Passengers who need assistance should speak to the desk."},{"scene":13,"file":"weather-delay.wav","label":"ANNOUNCEMENT B - PROBLEM AND ACTION","text":"Passengers on Flight sixty-one to Oslo: the flight is delayed by forty minutes because of weather. Please remain near Gate six and watch the information screen."},{"scene":22,"file":"check-in-dialogue.wav","label":"CHECK-IN DESK - RECORD THE DETAILS","text":"Good afternoon. May I see your passport and boarding pass? Here you are. Would you like an aisle seat or a window seat? A window seat, please. Your bag can be checked through to Rome."},{"scene":24,"file":"final-call.wav","label":"FINAL CALL - PASSENGERS, FLIGHT, AND GATE","text":"This is the final boarding call for passengers Karim Ali and Sofia Chen travelling to Madrid on Flight seven four three. Please proceed immediately to Gate twenty-one."}]};
  ...
  modal.innerHTML = '<div class="v3-lab__sheet"><button type="button" class="v3-close" aria-label="Close group task">CLOSE</button><span class="v3-lab__eyebrow">'+config.label+' - ONLINE GROUP TASK</span><h2>'+config.lab.title+'</h2><p class="v3-lab__prompt">'+config.lab.prompt+'</p><div class="v3-lab__grid"><div class="v3-role-grid">'+config.lab.roles.map((x,i)=>'<div class="v3-role"><strong>ROLE 0'+(i+1)+'</strong>'+x+'</div>').join('')+'</div><div><div class="v3-rounds"><strong>STEPS</strong>'+config.lab.rounds.join('<br>')+'</div><div class="v3-success"><strong>CHECK THE ANSWER</strong>'+config.lab.success+'</div></div></div></div>';
  root.append(modal);
